...
class SVM():
    def __init__(self, data, label, max_itera, C, tolar):
        ...
        self.w = mat(zeros([self.num, 1]))
        self.b = 0
        self.alpha = mat(zeros([1, self.num]))      # size is num of dataset
        self.tolar = tolar

    # type 1:line kernel; 2:RBF Kernel
    def kernel_fuction(self, x1, x2, type):
        ...
        for idx in range(self.max_itera):
            for i in range(self.num):
                ...
                if (E_i * self.label[:, i] > self.tolar and self.alpha[:, i] < self.C) \
                    ...
                    eta = self.kernel_fuction(self.data[i], self.data[j]) + self.kernel_fuction(self.data[j], self.data[j]) \
                        - 2 * self.kernel_fuction(self.data[i], self.data[j])
                    ...
                    b1_new = -E_i - self.label[i] * self.kernel_fuction(self.data[i], self.data[i]) * (self.alpha[i] - alphaold_i) \
    ...
    def calculate_w(self):
        ...
        for i in range(self.num):
            self.w += self.alpha[i] * self.label[i] * self.data[i].T
    ...
    def predict(self, test_data, label):
        ...
        for i in range(l):
            ...
            if sign(pre_value) != sign(label[i]):
                error += 1
        print("the acc:{}".format(len((l - error) / l)))


if __name__ == '__main__':
    data, label = load_data("/home/norman/Dataset/iris.txt")
